...
export default function HotelsCreate() {
    ...
    const handleSubmit = (e) => {
        ...
        if (imageFile) {
            ...
            router.post("/admin/hotels", formData, {
                forceFormData: true,
            });
        } else {
            router.post("/admin/hotels", form);
        }
    };

    return (
        <Layout>
            <Head title="Tambah Hotel — Admin GO360" />
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <h1 className="text-2xl font-display font-light text-primary-dark">Tambah Hotel Baru</h1>
                    <p className="text-xs text-forest-muted/60 mt-1">Destinasi terdekat akan otomatis terhubung setelah disimpan.</p>
                </div>
                                ...
                                {form.lat && form.lng && (
                                    <div className="p-4 rounded-xl bg-green-50 border border-green-200">
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <label className="text-[0.65rem] text-green-700 block mb-1">Latitude</label>
                                                <input type="text" readOnly value={form.lat} className="w-full bg-transparent border-0 text-sm text-green-800 font-mono p-0" />
                                            </div>
                                            <div>
                                                <label className="text-[0.65rem] text-green-700 block mb-1">Longitude</label>
                                                <input type="text" readOnly value={form.lng} className="w-full bg-transparent border-0 text-sm text-green-800 font-mono p-0" />
                                            </div>
                                        </div>
                                    </div>
                                )}
                                <p className="text-[0.65rem] text-forest-muted/60 italic">
                                    Destinasi terdekat akan otomatis terhubung setelah disimpan.
                                </p>
                            </div>
                        </div>
                    </div>
